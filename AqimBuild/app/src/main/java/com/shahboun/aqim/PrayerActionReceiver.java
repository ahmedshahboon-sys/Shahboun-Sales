...
public class PrayerActionReceiver extends BroadcastReceiver {
    ...
    @Override public void onReceive(Context c, Intent i){
        ...
        if(ACTION_DONE.equals(i.getAction())){
            c.getSharedPreferences("aqim",Context.MODE_PRIVATE).edit().putBoolean("done_"+dayKey()+"_"+prayer,true).apply();
            ((NotificationManager)c.getSystemService(Context.NOTIFICATION_SERVICE)).cancel(notificationId(prayer)); return;
        }
    ...
    public static void showQuestion(Context c,String prayer,int attempt){
        ...
        PendingIntent dp=PendingIntent.getBroadcast(c,5000+Math.abs(prayer.hashCode()%1000),done,PendingIntent.FLAG_UPDATE_CURRENT|PendingIntent.FLAG_IMMUTABLE);
        PendingIntent sp=PendingIntent.getBroadcast(c,6000+Math.abs(prayer.hashCode()%1000),snooze,PendingIntent.FLAG_UPDATE_CURRENT|PendingIntent.FLAG_IMMUTABLE);
        Notification.Builder b=Build.VERSION.SDK_INT>=26?new Notification.Builder(c,CH):new Notification.Builder(c);
        Notification n=b.setSmallIcon(android.R.drawable.ic_popup_reminder).setContentTitle("هل صليت "+prayer+"؟").setContentText("أكد الصلاة أو أجّل التذكير حتى لا تفوتك")
                .setAutoCancel(false).setPriority(Notification.PRIORITY_HIGH).addAction(0,"نعم، صليت",dp).addAction(0,"ذكّرني لاحقًا",sp).build();
    ...
    public static void scheduleQuestion(Context c,String prayer,int attempt,long when){
        ...
        int day=(int)((when/86400000L)%1000L); int code=700000+day*100+Math.abs(prayer.hashCode()%70)+attempt;
