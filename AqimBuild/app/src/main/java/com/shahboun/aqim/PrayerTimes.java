...
public final class PrayerTimes {
    private PrayerTimes() {}
    // Local calculation is the offline source of truth. Libya offsets are optional calibration only.
    ...
    public static LinkedHashMap<String, Calendar> calculate(Calendar date,double lat,double lon,int[] offsets){
        return calculate(date,lat,lon,offsets,true,false);
    }
    public static LinkedHashMap<String, Calendar> calculate(Calendar date,double lat,double lon,int[] offsets,boolean shafii,boolean libyaCalibration){
        ...
        for(int pass=0;pass<2;pass++){
            double[] p=new double[t.length]; for(int i=0;i<t.length;i++)p[i]=t[i]/24.0;
            double fajr=sunAngleTime(jd,lat,19.5,p[0],true);
            ...
            double isha=sunAngleTime(jd,lat,17.5,p[6],false);
        ...
        boolean libya=libyaCalibration&&"Africa/Tripoli".equals(tz.getID())&&lat>=19&&lat<=34&&lon>=9&&lon<=26;
    ...
    private static double sin(double d){return Math.sin(Math.toRadians(d));}private static double cos(double d){return Math.cos(Math.toRadians(d));}private static double tan(double d){return Math.tan(Math.toRadians(d));}private static double arcsin(double x){return Math.toDegrees(Math.asin(x));}private static double arccos(double x){return Math.toDegrees(Math.acos(x));}private static double arctan2(double y,double x){return Math.toDegrees(Math.atan2(y,x));}private static double arccot(double x){return Math.toDegrees(Math.atan2(1.0,x));}private static double fixAngle(double a){a-=360*Math.floor(a/360);return a<0?a+360:a;}private static double fixHour(double a){a-=24*Math.floor(a/24);return a<0?a+24:a;}
}
