...
public class AqimStatusService extends Service {
    ...
    private Notification build(){
        String[] n=MainActivity.nextPrayerInfo(this);
        Intent open=new Intent(this,DashboardActivity.class);
        PendingIntent pi=PendingIntent.getActivity(this,3101,open,PendingIntent.FLAG_UPDATE_CURRENT|PendingIntent.FLAG_IMMUTABLE);
        Notification.Builder b=Build.VERSION.SDK_INT>=26?new Notification.Builder(this,CH):new Notification.Builder(this);
        String iq=n.length>4?" • "+n[4]:"";
        return b.setSmallIcon(android.R.drawable.ic_lock_idle_alarm).setContentTitle("أَقِم • "+n[0]+" "+n[1]).setContentText("باقي "+n[2]+iq+" • "+n[3]).setContentIntent(pi).setOngoing(true).setOnlyAlertOnce(true).setVisibility(Notification.VISIBILITY_PUBLIC).build();
    }
