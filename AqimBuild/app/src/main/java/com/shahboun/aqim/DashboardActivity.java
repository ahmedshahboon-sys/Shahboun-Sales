...
public class DashboardActivity extends Activity {
    ...
    void show(){LinearLayout root=new LinearLayout(this);root.setOrientation(LinearLayout.VERTICAL);root.setGravity(Gravity.CENTER_HORIZONTAL);root.setPadding(dp(18),dp(16),dp(18),dp(30));root.setBackgroundColor(IVORY);root.setLayoutDirection(View.LAYOUT_DIRECTION_RTL);
        ...
        GridLayout grid=new GridLayout(this);grid.setColumnCount(getResources().getConfiguration().smallestScreenWidthDp>=600?3:2);
        String[] labels={"الصلاة والأذان","المصحف والروايات","عبادتي اليوم","الموقع العالمي","القبلة الحية","تذكير الذكر","المسبحة والفقاعة","الإعدادات"};
        for(int i=0;i<labels.length;i++){Button x=b(labels[i],false);final int k=i;x.setOnClickListener(v->openFeature(k));GridLayout.LayoutParams gp=new GridLayout.LayoutParams();gp.width=0;gp.height=dp(84);gp.columnSpec=GridLayout.spec(GridLayout.UNDEFINED,1f);gp.setMargins(dp(6),dp(6),dp(6),dp(6));grid.addView(x,gp);}root.addView(grid,new LinearLayout.LayoutParams(-1,-2));
    ...
    void openFeature(int k){Intent in;if(k==0)in=new Intent(this,MainActivity.class).putExtra("target","prayers");else if(k==1)in=new Intent(this,QuranActivity.class);else if(k==2)in=new Intent(this,IbadatiActivity.class);else if(k==3)in=new Intent(this,WorldLocationActivity.class);else if(k==4)in=new Intent(this,LiveQiblaActivity.class);else if(k==5)in=new Intent(this,DhikrSettingsActivity.class);else if(k==6)in=new Intent(this,MainActivity.class).putExtra("target","tasbih");else in=new Intent(this,MainActivity.class).putExtra("target","settings");startActivity(in);}
}
