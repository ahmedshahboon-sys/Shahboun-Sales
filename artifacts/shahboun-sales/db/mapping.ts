...
export const saleOps = (s: Sale): UpsertOp[] => [
  { table: 'sales', row: { id: s.id, invoice_number: s.invoiceNumber, created_at: s.createdAt, subtotal: s.subtotal, discount: s.discount, total: s.total, paid: s.paid, payment_method: s.paymentMethod, customer_id: s.customerId ?? null, user: s.user, returned: s.returned ?? false } },
...
export const rowToSale = (r: Row, items: Row[]): Sale => ({ id: r.id, invoiceNumber: r.invoice_number, createdAt: r.created_at, subtotal: r.subtotal, discount: r.discount, total: r.total, paid: r.paid, paymentMethod: r.payment_method as PaymentMethod, customerId: r.customer_id ?? undefined, user: r.user ?? 'admin', returned: !!r.returned, items: items.map((i) => ({ productId: i.product_id, name: i.name, quantity: i.quantity, unitPrice: i.unit_price, costPrice: i.cost_price, total: i.total })) });
